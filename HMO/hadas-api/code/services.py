...
def add_corona_info_service():
    ...
    date_vars = [first_vac, second_vac, third_vac, fourth_vac]
    # Validate all records
    valid, message = validate_records(
        customer_id=customer_id,
        text_vars=text_vars,
    ...
    # Activate DB
        ...
        cur.close()
        return jsonify(
            {"message": f"Customer's {customer_id} corona data added to database."}
        )
...
def query_record_service():
    ...
    cur = db.cursor()
    cur.execute(
        f"SELECT * FROM customer_data JOIN corona_data using(customer_id) WHERE {field} = '{value}' "
    )
...
def upload_file_service():
    ...
    db = get_db()
    cur = db.cursor()
    cur.execute(f"SELECT * FROM customer_data WHERE customer_id={id}")
    existing_person = cur.fetchone()
    if not existing_person:
